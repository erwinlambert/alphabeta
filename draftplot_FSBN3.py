...
FSmean = np.arange(.0005,.02+step,step)
BNmean = np.arange(.0005,.01+step,step)
rmean = .004
width = 5.e5
width2 = 2.5e5
xmax = 3.*L/5.
...
cax = ax[0].contourf(I,J,Psi1.T*1.e-6,10,cmap=plt.get_cmap('Reds'))
cbar = plt.colorbar(cax,ax=ax[0])
cbar.ax.set_title('[Sv]')

cax = ax[1].contourf(I,J,Psi0.T*1.e-6,10,cmap=plt.get_cmap('Purples'))
cbar = plt.colorbar(cax,ax=ax[1])
cbar.ax.set_title('[Sv]')

cax = ax[2].contourf(I,J,Psi2.T*1.e-6,10,cmap=plt.get_cmap('Blues'))
