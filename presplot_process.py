...
dax.plot(x,-fac*np.cumsum(mtl)*dx,color=pur,label='Eddies')
dax.plot(x,-fac*np.cumsum(mtv)*dx,'--',color=pur,label='Vert diff')
dax.plot(x,fac*np.cumsum(msl)*dx,color=blu,label='Eddies')
dax.plot(x,fac*np.cumsum(msv)*dx,'--',color=blu,label='Vert diff')
dax.legend(loc='lower left')

ax.set_ylabel('Volume transformation [Sv]')
...
fname = '../figures/presplot_process.png'
