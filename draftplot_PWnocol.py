...
dax.contourf(s,h,Psi2*1.e-6,cmap=cmap)
mag = (dHdx0**2+dSdx0**2)**n1
lw = n2+n3*mag/mag.max()
dax.streamplot(s,h,dSdx0,dHdx0,color='k',density=.8,arrowsize=2.)

#Make runoff plot
dax = ax[1]
#dax.contour(s,h,Psi2**n4,n0,colors='.5',linestyles='solid',linewidths=1.)
im = dax.contourf(s,h,Psi2*1.e-6,cmap=cmap)
mag = (dHdx1**2+dSdx1**2)**n1
lw = n2+n3*mag/mag.max()
dax.streamplot(s,h,dSdx1,dHdx1,color='k',density=.8,arrowsize=2.)
...
im = ax.contourf(s,h,Psi2*1.e-6,cmap=cmap)
dHdx = dHdx0+dHdx1
dSdx = dSdx0+dSdx1
mag = (dHdx**2+dSdx**2)**n1
lw = n2+n3*mag/mag.max()
ax.streamplot(s,h,dSdx,dHdx,color='k',density=.8,arrowsize=2.)
...
ax.plot(S2,Hsg,color='y',linewidth=4.,zorder=9)
ax.scatter(S2[0],Hsg[0],150,color='y',label='Inflow',zorder=9,clip_on=False)
ax.scatter(S2[-1],Hsg[-1],150,marker='s',color='y',label='Outflow',zorder=9,clip_on=False)
